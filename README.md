...
steht eine Streamlit-Browseroberfläche bereit.
...
`No module named streamlit` erscheinen. Nach erfolgreicher Installation kann mit
folgendem Befehl geprüft werden, welches Streamlit-Modul genau dieser Interpreter
...
Streamlit verwendet hier Port 8501. Codespaces zeigt nach dem Start eine
...
weil `streamlit_app.py` keine Qt-Desktopbibliotheken importiert.
...
- **PowerShell findet `python` nicht:** `py -3 -m streamlit run streamlit_app.py`
  verwenden und
  kontrollieren, ob die virtuelle Umgebung aktiv ist.
- **`libGL.so.1` in Codespaces:** Die Browseroberfläche mit
  `streamlit run streamlit_app.py` starten.
